...
- **Numbers.** Spell out zero through nine; numerals for 10 and above. No exception for scannable stats or metrics anymore — team size, page counts, results, and percentages spell out under 10 too ("a seven-person team," not "a 7-person team"), including in a `description` field or a "Key results" list. Two narrow exceptions remain, both about notation rather than magnitude, not judgment calls: "X+" shorthand for "or more" stays as written since spelling it out breaks the notation (e.g. "6+ cross-functional teams," not "six+"), and a mixed range where one number is 10 or above keeps both numerals for consistency (e.g. "5 to 10 files," not "five to 10").
...
- **List/table ordering.** When a list or table enumerates the same category of items — checks, skills, repo folders, anything readers might scan for a specific entry — keep it in a consistent, logical order, alphabetical by default unless sequence is inherently meaningful (e.g. sequential steps, which stay in process order, not alphabetical). Apply the same order everywhere that category of items is listed across the site, not just within one page.
